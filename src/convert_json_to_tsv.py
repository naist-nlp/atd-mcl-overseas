...
BEST_REF_STAT   = 'best_ref_status'
BEST_REF_AREA   = "best_ref_area_type"
SEC_A_REF_TYPE  = "second_A_ref_type"
SEC_A_REF_URL   = "second_A_ref_url"
SEC_A_REF_QUERY = "second_A_ref_query"
SEC_A_REF_STAT  = 'second_A_ref_status'
SEC_A_REF_AREA  = "second_A_ref_area_type"
SEC_B_REF_TYPE  = "second_B_ref_type"
SEC_B_REF_URL   = "second_B_ref_url"
SEC_B_REF_QUERY = "second_B_ref_query"
SEC_B_REF_STAT  = 'second_B_ref_status'
SEC_B_REF_AREA  = "second_B_ref_area_type"
...
def read_and_write(
    ...
    for doc_id, doc in data.items():
        ...
        for ent_id, ent in doc[ENTS].items():
            ...
            b_sta     = ent[BEST_REF_STAT]   if BEST_REF_STAT   in ent else ''
            ...
            s_a_sta   = ent[SEC_A_REF_STAT]  if SEC_A_REF_STAT  in ent else ''
            ...
            s_b_sta   = ent[SEC_B_REF_STAT]  if SEC_B_REF_STAT  in ent else ''

            b_os   = 'overseas' if BEST_REF_AREA  in ent and ent[BEST_REF_AREA]  else ''
    ...
    fw1.close()
    fw2.close()
    print(f'[Info] Saved: {output_tsv1_path}', file=sys.stderr)
    print(f'[Info] Saved: {output_tsv2_path}', file=sys.stderr)
